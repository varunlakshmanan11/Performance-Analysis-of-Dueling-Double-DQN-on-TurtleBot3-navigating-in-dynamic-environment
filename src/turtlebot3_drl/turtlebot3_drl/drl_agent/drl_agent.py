...
from ..q_learning_parameters.settings import ENABLE_VISUAL, ENABLE_STACKING, OBSERVE_STEPS, MODEL_STORE_INTERVAL, GRAPH_DRAW_INTERVAL

from ..q_learning_parameters.storagemanager import StorageManager
from ..q_learning_parameters.graph import Graph
from ..q_learning_parameters.logger import Logger
if ENABLE_VISUAL:
    from ..q_learning_parameters.visual import DrlVisual
from ..q_learning_parameters import utilities as util
...
from ..q_learning_parameters.replaybuffer import ReplayBuffer
...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0):
        ...
        if self.algorithm == 'dqn':
            ...
            self.model = DUELING_DQN(self.device, self.sim_speed)
        else:
            quit("\033[1m" + "\033[93m" + f"invalid algorithm specified ({self.algorithm}), choose one of: dqn or dueling_dqn" + "\033[0m}")
        ...
        if self.load_session:
            ...
            print(f"loaded model {self.load_session} (episode {self.episode}): {self.model.get_model_parameters()}")
        else:
    ...
    def finish_episode(self, step, eps_duration, outcome, dist_traveled, reward_sum, loss_critic, lost_actor):
            ...
            self.episode += 1
            print(f"Episode: {self.episode:<5}Reward: {reward_sum:<8.0f}outcome: {util.translate_outcome(outcome):<13}", end='')
            print(f"Steps: {step:<6}Total_steps: {self.total_steps:<7}time: {eps_duration:<6.2f}")
